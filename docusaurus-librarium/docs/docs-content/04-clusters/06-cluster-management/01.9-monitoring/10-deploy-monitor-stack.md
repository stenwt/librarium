...
<TabItem value="Without Authentication" label="without-auth">
...
<TabItem value="With Authentication and Encryption" label="with-auth">
...
![An architecture diagram that displays the network flow and infrastructure components](/assets/docs/images/clusters_monitoring_deploy-monitor-stack_https-architecture.png)
...
41. Create a Canonical Name (CNAME) record for each of the following services and add the load balancer hostname to the CNAME's record value. Use the table below to identify which mapping to use between the domain and each load balancer hostname.
  ...
  ![A screenshot of the Palette user interface with two boxes highlighting the load balancers that you need to add to your CNAME.](/assets/docs/images/clusters_monitoring_deploy-monitor-stack_loadbalancers.png)
